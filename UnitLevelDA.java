...
public class UnitLevelDA extends DVLClient{
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response) 
		...
		while (errorFound == false){
			...
			getAllRefDataRequest.setRefTable(JopesRefTablesTypes.UNIT_LEVEL_REF);
			...
			try{
				...
				if ( (getAllRefDataResponse == null) || (getAllRefDataResponse.getUnitLevelRef() == null) ){
					break;  //stop requesting data
				}
				
				System.out.println("Got data back from the service.");

				List<UnitLevelRef> ccRefs = getAllRefDataResponse.getUnitLevelRef();
		...
		if (errorMsg != null){
		  ...
		  out.println("<h1>" + "Saved " + numReceived + " UnitLevelRef objects." + "</h1>");
		}
